...
def analyze_file_structure(file_path: str) -> Dict[str, any]:
    ...
    try:
        ...
        if len(date_parts) >= 2:
            ...
            date_indices.sort()
            is_consecutive = all(
                date_indices[i] + 1 == date_indices[i + 1]
...
for file_type, file_list in discovered_files.items():
    ...
    for file_path in sample_files:
        ...
        if table_name_candidates:
            ...
            if "synthetic_data" in file_path or any(
                "retail_oltp" in part for part in path_parts
            ):
        ...
        file_info["inferred_table_name"] = (
            table_name.replace("-", "_").replace(" ", "_").lower()
        )
...
def generate_ingestion_config(file_info: Dict, config_counter: int) -> Dict:
    ...
    config = {
        ...
        "import_pattern": file_info.get(
            "import_pattern",
            "date_partitioned" if file_info["is_partitioned"] else "single_file",
        ),
        "date_partition_format": file_info.get("date_pattern"),
        "table_relationship_group": f"auto_discovered_{datetime.now().strftime('%Y%m%d')}",
        "batch_import_enabled": file_info.get(
            "batch_import_enabled", file_info["is_partitioned"]
        ),
        "file_discovery_pattern": file_info.get(
            "file_discovery_pattern",
            f"**/*.{file_info['format']}" if file_info["is_partitioned"] else None,
        ),
    ...
    for file_info in file_infos:
        if "error" not in file_info:
            ...
            if file_info.get("date_components"):
                grouped_configs[table_name]["date_ranges"].append(
                    file_info["date_components"]
                )
...
for table_name, group_info in grouped_configs.items():
    ...
    if is_partitioned and len(files) > 1:
        ...
        if date_ranges:
            ...
            incremental_info["file_discovery_pattern"] = (
                f"**/*.{base_file_info['format']}"
            )
            ...
            config["config_name"] = (
                f"Auto-discovered incremental: {table_name} ({base_file_info['format']}) - {len(files)} date partitions"
            )
...
print(
    "# lh_utils.save_dataframe_as_table(configs_df, 'config_flat_file_ingestion', mode='append')"
)
...
print("")
print("📝 All Generated Configuration Details:")
print("=" * 120)
for i, config in enumerate(all_configs, 1):
    ...
    if config.get("date_range_start") and config.get("date_range_end"):
        print(
            f"    Date Range: {config['date_range_start']} to {config['date_range_end']}"
        )
    if config.get("file_discovery_pattern"):
        print(f"    Discovery Pattern: {config['file_discovery_pattern']}")
    if config.get("file_delimiter"):
        print(
            f"    CSV Settings: delimiter='{config['file_delimiter']}', header={config['has_header']}"
        )
    print("")
