...
def execute_notebook(notebook_name, index, total, timeout_seconds=3600):
    ...
    try:
        ...
        result = notebookutils.mssparkutils.notebook.run(
            notebook_name, timeout=timeout_seconds, params=params
        )
        ...
        # Record the failure but continue execution to get full picture
        failed_notebooks.append(
            {"name": notebook_name, "error": str(e), "index": index}
        )
        print(f"✗ Failed to execute: {notebook_name}")
        print(f"Error: {str(e)}")
        return False
...
print("Starting orchestration for  warehouse")
print(f"Start time: {start_time}")
print("Total notebooks to execute: 3")
print("=" * 60)
execute_notebook("000_Initial_Schema_Creation_Config_Warehouses_ddl_scripts", 1, 3)
execute_notebook("001_Initial_Creation_Ingestion_Config_Warehouses_ddl_scripts", 2, 3)
execute_notebook("002_Sample_Data_Ingestion_Config_Warehouses_ddl_scripts", 3, 3)
