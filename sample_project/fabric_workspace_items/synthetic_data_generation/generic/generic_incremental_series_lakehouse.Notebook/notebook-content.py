...
if run_mode == "local":
    ...
    from ingen_fab.python_libs.common.synthetic_data_dataset_configs import (
...
if isinstance(job_configs, dict):
    ...
    raise ValueError(
        "job_configs must be either a dictionary or a list of dictionaries"
    )
...
for idx, job_config in enumerate(job_configs_list):
    ...
    try:
        ...
        dataset_config = DatasetConfigurationRepository.get_predefined_dataset(
            dataset_id
        )
        incremental_config = DatasetConfigurationRepository.get_incremental_config(
            dataset_id
        )
        ...
        if enable_data_drift is not None:
            dataset_config["incremental_config"]["enable_data_drift"] = (
                enable_data_drift
            )
        ...
        if enable_seasonal_patterns is not None:
            dataset_config["incremental_config"]["enable_seasonal_patterns"] = (
                enable_seasonal_patterns
            )
        ...
        if generation_mode == "auto":
            ...
            generation_mode = (
                "pyspark" if total_estimated_rows > 1000000 else "pyspark"
            )  # Always use PySpark for lakehouse
        ...
        print(
            f"✅ Validated job '{job_name}': {dataset_config['dataset_name']} ({start_date} to {end_date})"
        )
...
base_generator = PySparkSyntheticDataGenerator(
    lakehouse_utils_instance=lh_utils, seed=seed_value
)
...
for idx, job in enumerate(valid_jobs, 1):
    ...
    print(
        f"{'Schema Pattern':<25}: {job['dataset_config'].get('schema_pattern', 'N/A')}"
    )
    ...
    for table_name, table_config in job["table_configs"].items():
        ...
        if table_type == "incremental":
            ...
            print(
                f"  {table_name:<20}: {table_type:<12} | {rows_per_day:>10,} rows/day"
            )
        else:
            base_rows = table_config.get("base_rows", 0)
            frequency = table_config.get("frequency", "unknown")
            print(
                f"  {table_name:<20}: {table_type:<12} | {base_rows:>10,} rows ({frequency})"
            )
print("\n" + "=" * 80)
...
for job in valid_jobs:
    ...
    while current_date <= job["end_dt"]:
        batch_end = min(
            current_date + timedelta(days=job["batch_size"] - 1), job["end_dt"]
        )
        job_batches.append(
            {
    ...
    all_job_batches.append({"job": job, "batches": job_batches})
    ...
    if len(job_batches) <= 5:  # Show all batches if 5 or fewer
        for i, batch in enumerate(job_batches, 1):
            print(
                f"    Batch {i}: {batch['start_date']} to {batch['end_date']} ({batch['days']} days)"
            )
    else:  # Show first 3 and last 2 if more than 5
        for i in range(3):
            batch = job_batches[i]
            print(
                f"    Batch {i + 1}: {batch['start_date']} to {batch['end_date']} ({batch['days']} days)"
            )
        print("    ...")
...
for job_idx, job_batch_info in enumerate(all_job_batches, 1):
    ...
    for batch_num, batch in enumerate(job_batches, 1):
        ...
        while current_date <= batch["end_date"]:
            ...
            for table_name, table_config in job["table_configs"].items():
                ...
                if table_type == "incremental":
                    ...
                    if job["enable_seasonal_patterns"] and table_config.get(
                        "seasonal_multipliers_enabled", False
                    ):
                        day_of_week = current_date.strftime("%A").lower()
                        seasonal_multipliers = job["incremental_config"].get(
                    ...
                    if table_name == "customers":
                        table_df = generator._generate_customers_incremental(
                            adjusted_rows, current_date, state
                        )
                    elif table_name == "products":
                        table_df = generator._generate_products_incremental(
                            adjusted_rows, current_date, state
                        )
                    elif table_name == "orders":
                        # For orders, use incremental generator with proper date
                        table_df = generator._generate_orders_incremental(
                            adjusted_rows, current_date, state
                        )
                    elif table_name == "order_items":
                        # For order items, use incremental generator
                        table_df = generator._generate_order_items_incremental(
                            adjusted_rows, current_date, state
                        )
                    else:
                        # Generic table generation
                        table_df = generator._generate_generic_table_incremental(
                            table_name, adjusted_rows, current_date, state
                        )
                    ...
                    if job["output_mode"] == "table":
                        ...
                        print(
                            f"    ✅ {table_name}: {table_df.count():,} rows → table '{final_table_name}'"
                        )
                        ...
                        print(
                            f"    ✅ {table_name}: {table_df.count():,} rows → {file_path}"
                        )
                    ...
                    if frequency == "daily":
                        ...
                        frequency == "weekly" and current_date.weekday() == 0
                    ):  # Monday
                        should_generate = True
                    elif (
                        frequency == "monthly" and current_date.day == 1
                    ):  # First of month
                        should_generate = True
                    ...
                    if should_generate:
                        ...
                        table_df = generator._generate_generic_snapshot(
                            table_name, base_rows, current_date
                        )
                        batch_tables[table_name] = table_df
                        ...
                        if job["output_mode"] == "table":
                            ...
                            print(
                                f"    ✅ {table_name} (snapshot): {table_df.count():,} rows → table '{snapshot_table_name}'"
                            )
                        elif job["output_mode"] in ["parquet", "csv"]:
                            ...
                            print(
                                f"    ✅ {table_name} (snapshot): {table_df.count():,} rows → {file_path}"
                            )
        ...
        print(
            f"✅ Batch {batch_num} complete: {batch_rows:,} rows in {batch_duration:.1f}s"
        )
...
print("-" * 100)
print(
    f"{'Job Name':<30} {'Dataset':<25} {'Date Range':<25} {'Rows':<12} {'Duration':<10}"
)
print("-" * 100)
...
if len(valid_jobs) == 1:
    ...
    for job_result in job_results:
        ...
        avg_rows_per_day = (
            job_result["total_rows"] / total_days if total_days > 0 else 0
        )
        avg_rows_per_batch = (
            job_result["total_rows"] / job_result["total_batches"]
            if job_result["total_batches"] > 0
            else 0
        )
        print(f"\n{job_result['job_name']}:")
        ...
        print(
            f"  • Processing rate: {job_result['total_rows'] / job_result['duration']:,.0f} rows/second"
        )

print("=" * 80)
print(
    f"⚡ Overall processing rate: {total_rows_generated / execution_time:,.0f} rows/second"
)

# Log completion
if "notebookutils" in globals() and hasattr(notebookutils, "log"):
    notebookutils.log(
        f"Incremental series generation completed: {len(valid_jobs)} jobs, {total_rows_generated:,} rows, {execution_time:.1f}s"
    )
