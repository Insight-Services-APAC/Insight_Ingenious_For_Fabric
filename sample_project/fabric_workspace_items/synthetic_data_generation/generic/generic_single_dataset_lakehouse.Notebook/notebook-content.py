    ...
    try:
        ...
        if custom_schema:
            try:
                ...
                print(
                    f"⚠️ Warning: Invalid custom schema JSON for '{job_name}', ignoring: {e}"
                )

        # Determine optimal generation mode
        if generation_mode == "auto":
            generation_mode = (
                "pyspark" if target_rows > 1000000 else "pyspark"
        ...
        validated_job = {
            ...
            if "dim_customer_rows" in locals()
            else 0,
            "dim_product_rows": dim_product_rows
            if "dim_product_rows" in locals()
            else 0,
        ...
        print(
            f"✅ Validated job '{job_name}': {dataset_config['dataset_name']} ({target_rows:,} target rows)"
        )
...
for job_idx, job in enumerate(valid_jobs, 1):
    ...
    if job["seed_value"] is not None:
        generator = PySparkSyntheticDataGenerator(
            lakehouse_utils_instance=lh_utils, seed=job["seed_value"]
        )
        dataset_builder = PySparkDatasetBuilder(generator)
    ...
    if schema_pattern == "oltp":
        ...
        if (
            "orders" in job["dataset_config"].get("tables", [])
            and "customers" in generated_tables
        ):
        ...
        if (
            "order_items" in job["dataset_config"].get("tables", [])
            and "orders" in generated_tables
        ):
            print("  📦 Generating order items table...")
            order_items_df = generator.generate_order_items_table(
                generated_tables["orders"], generated_tables.get("products")
            )
        ...
        for dim in job["dataset_config"].get("dimensions", []):
            ...
            if dim == "dim_date":
                ...
                dim_df = generator.generate_customers_table(
                    job["dim_store_rows"]
                )  # Generic store data
            else:
                continue
        ...
        for fact in job["dataset_config"].get("fact_tables", []):
            ...
            if fact == "fact_sales":
                fact_df = generator.generate_fact_sales(
                    job["fact_sales_rows"], dimensions
                )
    ...
    if job["output_mode"] == "table":
        ...
        for table_name, table_df in generated_tables.items():
            ...
            if job["enable_partitioning"] and table_name in [
                "orders",
                "order_items",
                "fact_sales",
            ]:
                ...
                partition_cols = (
                    ["order_date"] if "order_date" in table_df.columns else None
                )
                ...
                print(
                    f"    ✅ Saved {prefixed_table_name} (partitioned) with {table_df.count():,} rows"
                )
            else:
                prefixed_table_name = f"synth_data_{job['job_name']}_{table_name}"
                lh_utils.write_to_table(table_df, prefixed_table_name, mode=write_mode)
                print(
                    f"    ✅ Saved {prefixed_table_name} with {table_df.count():,} rows"
                )

    elif job["output_mode"] == "parquet":
        print("  📄 Saving as Parquet files...")
        output_path = (
            f"synthetic_data/parquet/single/{job['dataset_id']}/{job['job_name']}"
        )
        generator.export_to_parquet(generated_tables, output_path)
        for table_name, table_df in generated_tables.items():
            print(
                f"    ✅ Saved {output_path}/{table_name}.parquet with {table_df.count():,} rows"
            )
        ...
        for table_name, table_df in generated_tables.items():
            print(
                f"    ✅ Saved {output_path}/{table_name}.csv with {table_df.count():,} rows"
            )
...
print("-" * 100)
print(
    f"{'Job Name':<25} {'Dataset':<20} {'Target':<10} {'Actual':<10} {'Tables':<8} {'Time':<8} {'Output'}"
)
print("-" * 100)
for result in job_results:
    print(
        f"{result['job_name']:<25} {result['dataset_id']:<20} {result['target_rows']:>9,} {result['actual_rows']:>9,} {result['tables_generated']:>7} {result['execution_time']:>7.1f}s {result['output_mode']}"
    )

print("-" * 100)
print(
    f"{'TOTALS':<25} {'':<20} {sum(r['target_rows'] for r in job_results):>9,} {total_rows_generated:>9,} {sum(r['tables_generated'] for r in job_results):>7} {execution_time:>7.1f}s"
)
...
if "notebookutils" in globals() and hasattr(notebookutils, "log"):
    notebookutils.log(
        f"Multi-job synthetic data generation completed: {len(job_results)} jobs, {total_rows_generated:,} total rows, {execution_time:.1f}s"
    )
