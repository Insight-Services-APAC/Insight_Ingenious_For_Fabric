...
class DocumentationValidator:
    ...
    def validate_import_statements(self, file_path: Path) -> List[str]:
        ...
        for block in python_blocks:
            # Check for imports
            imports = re.findall(r'^(?:from|import)\s+(.+?)(?:\s+import|\s*$)', block, re.MULTILINE)
            for imp in imports:
                ...
                if 'ingen_fab' in imp or any(lib in imp for lib in ['lakehouse_utils', 'warehouse_utils', 'ddl_utils']):
                    # For injected libraries (without ingen_fab prefix), these are valid in notebook context
                    if not imp.startswith('ingen_fab') and any(lib in imp for lib in ['lakehouse_utils', 'warehouse_utils', 'ddl_utils']):
                    ...
                    if 'common' in imp or 'python' in imp or 'pyspark' in imp:
                        if not imp.startswith('ingen_fab.python_libs'):
                            errors.append(f"Import should start with 'ingen_fab.python_libs': {imp}")
...
class TestDocumentationAccuracy:
    ...
    def test_cli_commands_exist(self, validator):
        ...
        expected_commands = ['deploy', 'init', 'ddl', 'test', 'notebook', 'package', 'libs']
        ...
        for doc_file in docs_files:
            ...
            for pattern in nonexistent_patterns:
                if re.search(pattern, content):
                    errors.append(f"Found non-existent pattern '{pattern}' in {doc_file}")
    ...
    def test_ddl_compile_options(self, validator):
        ...
        for doc_file in docs_files:
            ...
            ddl_commands = re.findall(r'ingen_fab ddl compile.*?(?=\n(?![\s\\]))', content, re.DOTALL)
