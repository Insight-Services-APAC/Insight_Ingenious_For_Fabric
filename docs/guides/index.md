# Guides

[Home](../index.md) > Guides

Additional guides and workflow patterns for using the Ingenious Fabric Accelerator effectively.

## Available Guides

- **[Workflows](../user_guide/workflows.md)** - Development and deployment workflow patterns
- **[Troubleshooting](troubleshooting.md)** - Common issues and solutions  
- **[Packages](../packages/index.md)** - Working with solution packages
- **[Deployment](../user_guide/deploy_guide.md)** - Deployment strategies and practices
- **[Common Tasks](../user_guide/common_tasks.md)** - Quick reference for frequent operations
- **[CI/CD Deployment](cicd-deployment.md)** - DevOps and continuous integration
- **[GitHub Actions Setup](github_configuration.md)** - GitHub Actions configuration and deployment

## Guide Categories

### **Development Workflows**
- [Workflows](../user_guide/workflows.md) - Complete development lifecycle patterns
- [Common Tasks](../user_guide/common_tasks.md) - Day-to-day operations

### **Deployment & Operations**  
- [Deployment](../user_guide/deploy_guide.md) - Standard deployment practices
- [CI/CD Deployment](cicd-deployment.md) - Automated deployment patterns
- [GitHub Actions Setup](github_configuration.md) - Github actions setup

### **Solutions & Packages**
- [Packages](../packages/index.md) - Using and creating solution packages

### **Support**
- [Troubleshooting](troubleshooting.md) - Problem resolution

---

!!! note "Guides vs User Guide"
    These **Guides** provide workflow patterns and best practices. For step-by-step instructions and command reference, see the **[User Guide](../user_guide/index.md)**.