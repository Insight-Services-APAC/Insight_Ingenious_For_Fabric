...
class GeneratePlatformTests:
    ...
    def _inject_python_libs_into_template(self) -> None:
        ...
        ConsoleStyles.print_success(self.console, "Starting injection of python libs into template.")
        ...
        ConsoleStyles.print_dim(self.console, f"Looking for language directories in: {tests_path}")
        ...
        for lang_dir in language_dirs:
            ...
            ConsoleStyles.print_warning(self.console, f"Processing language directory: {lang_dir}")
            libs_to_include: list[str] = None

            gpl = GatherPythonLibs(console=self.console, include_jinja_raw_tags=False)

            ConsoleStyles.print_dim(self.console, f"Gathering files for {lang_dir.name}")
            ...
            ConsoleStyles.print_dim(self.console, f"Searching for test scripts in: {test_scripts_dir}")
            found_files = list(test_scripts_dir.glob("*.py")) if test_scripts_dir.exists() else []
            ConsoleStyles.print_dim(
                self.console,
            ...
            for test_file in found_files:
                ConsoleStyles.print_dim(self.console, f"Adding test script: {test_file}")
            ...
            ConsoleStyles.print_dim(self.console, f"Loading template from: {test_template_name}")
            nb_template: Template = notebook_utils.load_template(test_template_name)
            nb_content = nb_template.render(
                library_bundle="\n".join(library_bundle),
                test_scripts="\n\n".join(test_scripts),
            )
            ConsoleStyles.print_success(self.console, f"Creating notebook for {lang_dir.name} in {output_dir}")
            notebook_utils.create_notebook_with_platform(
                notebook_name=f"{lang_dir.name}_platform_test",
                rendered_content=nb_content,
                output_dir=output_dir,
            )
        ConsoleStyles.print_success(self.console, "Completed injection of python libs into templates.")
