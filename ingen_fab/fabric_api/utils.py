...
class FabricApiUtils:
    ...
    def get_item_definition(
        self, workspace_id: str, item_id: str, item_type: str, format: str = "Default"
    ) -> Optional[dict]:
        """
        ...
        if response.status_code == 200:
            ...
            if operation_id:
                return self._poll_long_running_operation(
                    workspace_id, item_id, operation_id, location_url
                )
        else:
            ...
            try:
                ...
                if "error" in error_json:
                    ...
                    error_details += (
                        f", Code: {error_json['error'].get('code', 'Unknown code')}"
                    )
                else:
            ...
            raise Exception(
                f"Failed to get definition for item {item_id}. {error_details}"
            )
        return None
        ...
        for attempt in range(max_attempts):
            ...
            if response.status_code == 200:
                ...
                if status == "succeeded":
                    ...
                    raise Exception(
                        f"Operation failed: {error_details.get('message', 'Unknown error')}"
                    )
                    ...
                    if location_response.status_code == 200:
                        ...
                        if location_result.get("status", "").lower() == "succeeded":
                            ...
                            if result_response.status_code == 200:
                                result_data = result_response.json()
                                if result_data and (
                                    "definition" in result_data
                                    or "parts" in result_data
                                ):
                                    return result_data
                        ...
                        if (
                            "definition" in location_result
                            or "parts" in location_result
                        ):
    ...
    def download_item_content(
        self, workspace_id: str, item_id: str, item_type: str
    ) -> dict:
        """
        ...
        if item_type == "Notebook":
            try:
                definition = self.get_item_definition(
                    workspace_id, item_id, item_type, format="IPYNB"
                )
                if definition:
                    return {
