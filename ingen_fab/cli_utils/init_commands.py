...
def init_solution(project_name: str | None, path: Path):
    ...
    if project_name is None:
        ...
        if fabric_workspace_repo_dir:
            ...
            ConsoleStyles.print_info(console, f"Using FABRIC_WORKSPACE_REPO_DIR for project: {project_name} at {project_path}")
        else:
            ConsoleStyles.print_error(console, "❌ Project name is required when FABRIC_WORKSPACE_REPO_DIR is not set.")
            ConsoleStyles.print_info(console, "💡 Either set FABRIC_WORKSPACE_REPO_DIR or use --project-name option.")
    ...
    ConsoleStyles.print_info(console, "2. Update variable values in fabric_workspace_items/config/var_lib.VariableLibrary/valueSets/")
    ...
    try:
        ...
        if not workspace_id:
            if create_if_not_exists:
                ConsoleStyles.print_info(console, f"Workspace '{workspace_name}' not found. Creating new workspace...")
                try:
                    workspace_id = fabric_api.create_workspace(workspace_name)
                    workspace_created = True
                except Exception as e:
                    if "already exists" in str(e):
                        ConsoleStyles.print_error(console, f"❌ Workspace '{workspace_name}' already exists but was not found in the list")
                        ConsoleStyles.print_info(console, "💡 This might be a permissions issue or the workspace might be in a different capacity")
                    ConsoleStyles.print_error(console, f"❌ Failed to create workspace: {str(e)}")
                    raise typer.Exit(code=1)
            else:
                ConsoleStyles.print_error(console, f"❌ Workspace '{workspace_name}' not found in your Fabric environment.")
                ConsoleStyles.print_info(console, "💡 Make sure you have access to the workspace and the name is spelled correctly.")
                ConsoleStyles.print_info(console, f"💡 Or use --create-if-not-exists flag to create the workspace: ingen_fab init workspace --workspace-name '{workspace_name}' --create-if-not-exists")
                raise typer.Exit(code=1)
        
        if workspace_created:
            ConsoleStyles.print_success(console, f"✓ Created workspace '{workspace_name}' with ID: {workspace_id}")
        else:
        ...
        if is_single_workspace:
            ...
            ConsoleStyles.print_info(console, "You will need to manually configure the following workspace IDs in:")
            ConsoleStyles.print_info(console, f"  {project_path}/fabric_workspace_items/config/var_lib.VariableLibrary/valueSets/{environment}.json")
            ConsoleStyles.print_info(console, "\nWorkspace IDs to configure:")
            ConsoleStyles.print_info(console, "  - fabric_deployment_workspace_id (already set)")
...
def _update_valueset_workspace_id(project_path: Path, environment: str, workspace_id: str, console: Console):
    ...
    valueset_path = project_path / "fabric_workspace_items" / "config" / "var_lib.VariableLibrary" / "valueSets" / f"{environment}.json"
    
    if not valueset_path.exists():
        ConsoleStyles.print_error(console, f"❌ ValueSet file not found: {valueset_path}")
        ConsoleStyles.print_info(console, "💡 Make sure you're running this command from a valid Fabric project directory.")
        raise typer.Exit(code=1)
    ...
    try:
        ...
        if "variableOverrides" in valueset_data:
            ...
            if workspace_id_override:
                ...
                ConsoleStyles.print_info(console, f"✓ Updated fabric_deployment_workspace_id in {environment}.json")
                ...
                ConsoleStyles.print_error(console, "❌ fabric_deployment_workspace_id not found in variableOverrides")
                raise typer.Exit(code=1)
        else:
        ...
        if "variableOverrides" in valueset_data:
            ...
            for override in variable_overrides:
                ...
                if var_name in workspace_id_vars:
                    ...
                    if old_value != workspace_id:
                        ConsoleStyles.print_info(console, f"  ✓ Updated {var_name}: {old_value} → {workspace_id}")
                    else:
            ...
            if missing_vars:
                ConsoleStyles.print_warning(console, f"  ⚠️  Some workspace ID variables were not found: {', '.join(missing_vars)}")
...
def _check_and_update_artifacts(fabric_api: FabricApiUtils, project_path: Path, environment: str, workspace_id: str, console: Console):
    ...
    try:
        ...
        ConsoleStyles.print_info(console, f"  Found {len(lakehouses)} lakehouses and {len(warehouses)} warehouses")
