...
def compile_specific_file(
    ...
    try:
        ...
        if was_updated:
            console.print(
                f"[green]✓ Successfully compiled: {target_file} with values from {environment} environment[/green]"
            )
        else:
...
def compile_default_config_utils(ctx: typer.Context, environment: str) -> None:
    ...
    try:
        ...
        if was_updated:
            console.print(
                f"[green]✓ Successfully compiled config_utils.py with values from {environment} environment[/green]"
            )
        else:
