...
class ErrorCategorizer:
    ...
    def __init__(self):
        ...
        self.network_regex = re.compile(
            "|".join(self.NETWORK_ERROR_PATTERNS), 
            re.IGNORECASE
        )
        self.resource_regex = re.compile(
            "|".join(self.RESOURCE_ERROR_PATTERNS), 
            re.IGNORECASE
        )
        self.transient_regex = re.compile(
            "|".join(self.TRANSIENT_OPERATION_PATTERNS), 
            re.IGNORECASE
        )
        self.config_regex = re.compile(
            "|".join(self.CONFIG_ERROR_PATTERNS), 
            re.IGNORECASE
        )
        self.permanent_regex = re.compile(
            "|".join(self.PERMANENT_ERROR_PATTERNS), 
            re.IGNORECASE
        )
    ...
    def categorize_exception(
        ...
        if exception_type in ["ConnectionError", "ConnectTimeout", "ReadTimeout", "Timeout"]:
            return ErrorCategory.NETWORK, ErrorSeverity.MEDIUM, True
    ...
    def should_retry(
        ...
        if category in [ErrorCategory.NETWORK, ErrorCategory.TRANSIENT, ErrorCategory.RESOURCE]:
            return True
