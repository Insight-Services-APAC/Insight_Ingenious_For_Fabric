...
if _DEPENDENCIES_AVAILABLE:
    ...
    class SyntheticDataGenerator(ISyntheticDataGenerator):
        ...
        def generate_orders_table(self, num_rows: int, customer_ids: List[int], start_id: int = 1) -> pd.DataFrame:
            ...
            for i in range(num_rows):
                ...
                if status in ["Shipped", "Delivered"]:
                    ...
                    delivered_date = (
                        shipped_date + timedelta(days=random.randint(1, 7)) if status == "Delivered" else None
                    )
                else:
                    shipped_date = None
                    delivered_date = None
        ...
        def generate_order_items_table(self, orders_df: pd.DataFrame, product_ids: List[int]) -> pd.DataFrame:
            ...
            for _, order in orders_df.iterrows():
                ...
                for product_id in order_products:
                    ...
                    order_item = {
                        ...
                        "discount_amount": round(line_total * random.uniform(0.0, 0.1), 2)
            ...
            for i in range(num_customers):
                ...
                for version in range(versions):
                    customer = {
                        ...
                        "expiry_date": self.fake.date_between(start_date="today", end_date="+1y")
    ...
    class DatasetBuilder:
        ...
        def build_retail_oltp_dataset(self, scale: str = "small") -> Dict[str, pd.DataFrame]:
            ...
            orders_df = self.generator.generate_orders_table(config["orders"], customers_df["customer_id"].tolist())
            order_items_df = self.generator.generate_order_items_table(orders_df, products_df["product_id"].tolist())
