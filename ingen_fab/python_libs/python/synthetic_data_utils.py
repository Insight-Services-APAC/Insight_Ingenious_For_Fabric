...
if _DEPENDENCIES_AVAILABLE:
    class SyntheticDataGenerator:
        ...
        def __init__(self, seed: Optional[int] = None, locale: str = 'en_US'):
            ...
            if pd is None:
                raise ImportError("pandas library is required. Install with: pip install pandas")
            if np is None:
        ...
        def generate_customers_table(self, num_rows: int, start_id: int = 1) -> pd.DataFrame:
        ...
        def generate_products_table(self, num_rows: int, start_id: int = 1) -> pd.DataFrame:
        ...
        def generate_orders_table(self, num_rows: int, customer_ids: List[int], 
            ...
            for i in range(num_rows):
                ...
                if status in ['Shipped', 'Delivered']:
                    ...
                    delivered_date = shipped_date + timedelta(days=random.randint(1, 7)) if status == 'Delivered' else None
                else:
                    shipped_date = None
                    delivered_date = None
                ...
                order = {
                    ...
                    'notes': self.fake.text(max_nb_chars=100) if random.random() < 0.2 else None
                }
                orders.append(order)
            
            return pd.DataFrame(orders)
        
        def generate_order_items_table(self, orders_df: pd.DataFrame, 
            ...
            for _, order in orders_df.iterrows():
                ...
                for product_id in order_products:
                    ...
                    order_item = {
                        ...
                        'discount_amount': round(line_total * random.uniform(0.0, 0.1), 2) if random.random() < 0.2 else 0.0
                    }
        ...
        def generate_star_schema_dimensions(self, config: Dict[str, Any]) -> Dict[str, pd.DataFrame]:
        ...
        def generate_fact_sales(self, num_rows: int, dimensions: Dict[str, pd.DataFrame]) -> pd.DataFrame:
            """Generate fact table for sales analytics."""
            facts = []
            
            # Get dimension keys
            date_keys = dimensions['dim_date']['date_key'].tolist() if 'dim_date' in dimensions else [20240101]
            customer_keys = dimensions['dim_customer']['customer_key'].tolist() if 'dim_customer' in dimensions else list(range(1, 1001))
            product_keys = dimensions['dim_product']['product_key'].tolist() if 'dim_product' in dimensions else list(range(1, 101))
            store_keys = dimensions['dim_store']['store_key'].tolist() if 'dim_store' in dimensions else [1]
            ...
            for i in range(num_rows):
                ...
                fact = {
                    ...
                    'discount_amount': round(sales_amount * random.uniform(0.0, 0.15), 2),
        ...
        def _generate_date_dimension(self, start_year: int = 2020, end_year: int = 2025) -> pd.DataFrame:
            ...
            for i in range(num_customers):
                ...
                for version in range(versions):
                    customer = {
                        ...
                        'expiry_date': self.fake.date_between(start_date='today', end_date='+1y') if version < versions - 1 else None,
        ...
        def export_to_parquet(self, dataframes: Dict[str, pd.DataFrame], output_dir: str):
            ...
            for table_name, df in dataframes.items():
                ...
                self.logger.info(f"Exported {table_name} with {len(df)} rows to {output_path}")
    ...
    class DatasetBuilder:
        ...
        def build_retail_oltp_dataset(self, scale: str = 'small') -> Dict[str, pd.DataFrame]:
        ...
        def build_retail_star_schema_dataset(self, fact_rows: int = 100000) -> Dict[str, pd.DataFrame]:
