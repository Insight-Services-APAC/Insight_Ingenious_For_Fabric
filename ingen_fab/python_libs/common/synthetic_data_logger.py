...
class TableGenerationMetrics:
    ...
    def get_date_correlation_info(self) -> str:
        ...
        primary_col = next(
            (col for col in self.date_columns if col.is_primary_date),
            self.date_columns[0],
        )
...
class SyntheticDataLogger:
    ...
    def analyze_dataframe_date_columns(
        self,
    ...
    def _analyze_pyspark_dataframe(
        self, df: DataFrame, table_name: str, expected_date_columns: List[str] = None
    ) -> List[DateColumnInfo]:
    ...
    def log_table_generation_start(
        self,
        table_name: str,
        table_type: str,
    ...
    def log_dataset_generation_summary(self, summary: DatasetGenerationSummary):
        ...
        for table_metrics in summary.table_metrics:
            ...
            if primary_date_col and primary_date_col.min_date:
                date_info = (
                    f" ({primary_date_col.min_date})"
                    if primary_date_col.min_date == primary_date_col.max_date
                    else f" ({primary_date_col.min_date} to {primary_date_col.max_date})"
                )
            else:
                date_info = ""
    ...
    def export_generation_logs(
        self,
        output_path: str,
        format: Literal["json", "csv"] = "json",  # noqa: F821
    ):
        ...
        try:
            if format == "json":
                with open(output_path, "w") as f:
                    json.dump(
                        [asdict(log) for log in self._generation_logs],
                        f,
                        indent=2,
                        default=str,
                    )
