...
try:
    ...
    from ingen_fab.python_libs.common.synthetic_data_config import (
        ConfigurationManager,  # noqa: F401
...
class IncrementalSyntheticDataGenerator:
            ...
            if self.enhanced_logging:
                ...
                self.enhanced_logger.log_table_generation_start(
                    ...
                    config_applied={
                        ...
                        else table_config.base_rows_per_day,
            ...
            if df_result is not None:
                ...
                if self.enhanced_logging:
                    table_metrics = self.enhanced_logger.create_table_metrics(
                        ...
                        base_rows_configured=table_config.base_rows
                        if table_config.table_type == "snapshot"
                        else table_config.base_rows_per_day,
                    )
    ...
    def _generate_snapshot_table(
        ...
        growth_enabled = table_config.get("growth_enabled", False)  # noqa: F841
        churn_enabled = table_config.get("churn_enabled", False)  # noqa: F841
    ...
    def _generate_enhanced_file_path(
        self,
        table_name: str,
        dataset_id: str,
        generation_date: date,
    ...
    def _generate_snapshot_table_enhanced(
        self,
        table_name: str,
    ...
    def _generate_incremental_table_enhanced(
        self,
        table_name: str,
        table_config: TableGenerationConfig,
        generation_date: date,
    ...
    def _load_state_from_table(self, dataset_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            state_records = state_df.filter(col("dataset_id") == dataset_id).collect()
