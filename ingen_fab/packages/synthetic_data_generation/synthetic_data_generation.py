...
try:
    ...
    from ...python_libs.common.synthetic_data_config import (
...
class SyntheticDataGenerationCompiler(BaseNotebookCompiler):
    ...
    def compile_predefined_dataset_notebook(
        ...
        print(
            f"   Then: python -m ingen_fab.cli package synthetic-data execute-with-parameters generic_single_dataset_{target_environment} --dataset-id {dataset_id} --target-rows {target_rows}"
        )
        print()
        ...
        if target_environment == "warehouse":
            ...
            script_mappings = {
                "Warehouses/Config/001_Initial_Creation_SyntheticData": [
                    (
                        "001_config_synthetic_data_datasets.sql",
                        "001_config_synthetic_data_datasets.sql",
                    ),
                    (
                        "002_config_synthetic_data_generation_jobs.sql",
                        "002_config_synthetic_data_generation_jobs.sql",
                    ),
                    (
                        "003_log_synthetic_data_generation.sql",
                        "003_log_synthetic_data_generation.sql",
                    ),
                    (
                        "004_sample_dataset_configurations.sql",
                        "004_sample_dataset_configurations.sql",
                    ),
                ]
            }
            ...
            script_mappings = {
                "Lakehouses/Config/001_Initial_Creation_SyntheticData": [
                    (
                        "001_config_synthetic_data_datasets.py",
                        "001_config_synthetic_data_datasets.py",
                    ),
                    (
                        "002_config_synthetic_data_generation_jobs.py",
                        "002_config_synthetic_data_generation_jobs.py",
                    ),
                    (
                        "003_log_synthetic_data_generation.py",
                        "003_log_synthetic_data_generation.py",
                    ),
                    (
                        "004_sample_dataset_configurations.py",
                        "004_sample_dataset_configurations.py",
                    ),
                ]
            }
    ...
    def compile_all_synthetic_data_notebooks(
        ...
        # Add DDL compilation
        compile_functions.append((self.compile_ddl_scripts, [target_environment], {}))
    ...
    def compile_all_enhanced_synthetic_data_notebooks(
        ...
        if not ENHANCED_CONFIG_AVAILABLE:
            ...
            return self.compile_all_synthetic_data_notebooks(
                target_environment=target_environment, output_mode=output_mode
            )
